...
@testset "obtaining the LP-solution" begin
    ...
    function cutcallback(cb_data)
        x_val = MOI.get(optimizer, MOI.CallbackVariablePrimal{SCIP.CutCbData}(cb_data), x)
        y_val = MOI.get(optimizer, MOI.CallbackVariablePrimal{SCIP.CutCbData}(cb_data), y)
        calls += 1
    end
