...
struct ComposedCutSelector{
    N,
    CS<:Union{AbstractVector{<:AbstractCutSelector},NTuple{N,AbstractCutSelector}},
} <: AbstractCutSelector
    cutsels::CS
end
...
function select_cuts(
    ...
    (retcode, nselectedcuts, result) =
        select_cuts(cutsel.cutsels[1], scip, cuts, forced_cuts, root, maxnslectedcuts)
    for idx = 2:length(cutsel.cutsels)
        if retcode != SCIP_OKAY || nselectedcuts == 0
            return (retcode, nselectedcuts, result)
        end
        (retcode, nselectedcuts, result) =
            select_cuts(cutsel.cutsels[idx], scip, cuts, forced_cuts, root, nselectedcuts)
    end
    return (retcode, nselectedcuts, result)
end
