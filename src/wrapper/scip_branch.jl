...
function SCIPfindBranchrule(scip, name)
    ccall((:SCIPfindBranchrule, libscip), Ptr{SCIP_BRANCHRULE}, (Ptr{SCIP}, Cstring), scip, name)
end
...
function SCIPcontainsExternBranchCand(scip, var)
    ccall((:SCIPcontainsExternBranchCand, libscip), UInt32, (Ptr{SCIP}, Ptr{SCIP_VAR}), scip, var)
end
