...
function setparameter!(m::SCIPMathProgModel, name::AbstractString, value::Bool)
    @assert isascii(name)
    _setBoolParam(m, pointer(name), Cint(value))
end

function setparameter!(m::SCIPMathProgModel, name::AbstractString, value::Int)
    @assert isascii(name)
...
function setparameter!(m::SCIPMathProgModel, name::AbstractString, value::Float64)
    @assert isascii(name)
    _setRealParam(m, pointer(name), Cdouble(value))
end

function setparameter!(m::SCIPMathProgModel, name::AbstractString, value::AbstractChar)
    @assert isascii(name)
    _setCharParam(m, pointer(name), Cchar(value))
end

function setparameter!(m::SCIPMathProgModel, name::AbstractString, value::AbstractString)
    @assert isascii(name) && isascii(value)
    _setStringParam(m, pointer(name), pointer(value))
end
